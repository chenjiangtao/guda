 ...
 */
package com.foodoon.monitor.agent.config.app;
...
public class CreateAppConfigProperties {
    ...
    public Properties createaAppConfig(String srcFile, String destLocation)
        ...
        loadDefaultProperties(oldProps, destLocation);
        ...
        if (Configration.APP_RUN_MODE_PROD.equalsIgnoreCase(runMode)) {
            ...
            writeProperties(oldProps, destLocation);
            return oldProps;
        } else {
            writeProperties(newProps, destLocation);
            return newProps;
        }
    }

    public Properties createaAppConfig(InputStream inputStream, String destLocation)
                                                                                    throws ParserConfigurationException,
                                                                                    SAXException,
                                                                                    IOException,
                                                                                    TransformerException {
        Properties newProps = new Properties();
        xmlResovle.resolveXmlToProperties(inputStream, newProps);
        Properties oldProps = new Properties();
        loadDefaultProperties(oldProps, destLocation);
        String runMode = String.valueOf(oldProps.get(Configration.APP_RUN_MODE));
        if (Configration.APP_RUN_MODE_PROD.equalsIgnoreCase(runMode)) {
    ...
    public void writeProperties(Properties props, String destLocation) {
        FileSystemResource fsr = new FileSystemResource(destLocation);
        try {
            store(props,
    ...
    public void loadDefaultProperties(Properties props, String destLocation) {
        try {
            File file = ResourceUtils.getFile(destLocation);
            if (file.exists()) {
                propertiesPersister.load(props, new InputStreamReader(new FileInputStream(file)));
            } else {
                logger.warn("在路径:[" + destLocation
                            + "]中无法找到monitor-agent.properties。创建新的monitor-agent.properties!");
            }
        } catch (Exception e) {
