 ...
 */
public class GarImpl {

    public final static Logger logger = LoggerFactory.getLogger(GarImpl.class);
    ...
    public static void main(String args[]) throws Exception {
        ...
        //        }
        long time = Math.round(uptime());
        System.out.println(time / 86400 + "天" + (time % 86400) / 3600 + "小时" + (time % 3600) / 60
                           + "分" + time % 60 + "秒");
    }
    ...
    public static long memoryUsed() throws SigarException {
        ...
        return mem.getUsed() / 1024L;

    }

    public static double uptime() throws SigarException {
        Sigar sigar = new Sigar();
        ...
        try {
            ...
            for (String name : nis) {
                try {
                    ...
                    if (hwaddr == null || "00:00:00:00:00:00".equals(hwaddr)) {
                        continue;
                    }
                    hwaddr = hwaddr.replaceAll(":", "");
                    if (!hwlist.contains(hwaddr)) {
                        hwlist.add(hwaddr);
                    }

                    System.out.println("发现网卡:" + ifconfig.getHwaddr() + ",IP:"
                                       + ifconfig.getAddress());
            ...
            for (String hwaddr : hwlist) {
                sb.append(hwaddr.toUpperCase());
            }
            //              hardSignMd5 = md5(sb.toString());  
            //              if(hardSignMd5 !=null){  
            //                  hardSignMd5 = hardSignMd5.toUpperCase();  
            //              }  
            //              logger.info("硬件特征码:"+ hardSignMd5);  
        } catch (Exception e) {
            logger.error("", e);
        }
    }
