...
public class Netstat extends SigarCommandBase {
    ...
    private static final String[] HEADER    = new String[] { "Proto", "Local Address",
    ...
    public void output(String[] args) throws SigarException {
        ...
        for (int i = 0; i < connections.length; i++) {
            ...
            items.add(proto);
            items.add(formatAddress(conn.getType(), conn.getLocalAddress(), conn.getLocalPort(),
                LADDR_LEN));
            items.add(formatAddress(conn.getType(), conn.getRemoteAddress(), conn.getRemotePort(),
                RADDR_LEN));
            items.add(state);
