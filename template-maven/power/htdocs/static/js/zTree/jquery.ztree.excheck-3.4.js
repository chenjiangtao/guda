...
(function($) {
	...
	_data = {
		...
		getCheckStatus : function(setting, node) {
			...
			var checkedKey = setting.data.key.checked, r = {
				checked : node[checkedKey],
				half : node.halfCheck ? node.halfCheck
						: (setting.check.chkStyle == consts.radio.STYLE ? (node.check_Child_State === 2)
								: (node[checkedKey] ? (node.check_Child_State > -1 && node.check_Child_State < 2)
				...
				for ( var i = 0, l = node[childKey].length; i < l; i++) {
					...
					if (setting.check.chkStyle == consts.radio.STYLE) {
						...
						if (cNode.nocheck === true) {
							...
							tmp = (cNode.check_Child_State === -1 || cNode.check_Child_State === 2) ? 2
									: 1;
						} else {
	...
	_handler = {
		onCheckNode : function(event, node) {
			...
			if (tools.apply(setting.callback.beforeCheck, [ setting.treeId,
					node ], true) == false)
				return true;
	...
	_view = {
		checkNodeRelation : function(setting, node) {
			...
			if (setting.check.chkStyle == r.STYLE) {
				...
				if (node[checkedKey]
					...
					view.setChangeParentNodeCheckBox(setting, node, true);
				}
				if (!node[checkedKey]
		...
		makeChkClass : function(setting, node) {
			...
			if (node.chkDisabled === true) {
				...
				fullStyle = node[checkedKey] ? ((node.check_Child_State === 2 || node.check_Child_State === -1) ? c.FULL
						: c.PART)
		...
		setChangeParentNodeCheckBox : function(setting, node, value, srcNode) {
			...
			if (node.parentTId) {
				...
				var checkedLength = 0;
				for ( var i = 0, l = pNodes.length; i < l; i++) {// 包括自己在内的兄弟节点个个数(根节点除外)
					if (pNodes[i][checkedKey]) {
						checkedLength++;
					}
				}
				if (pNodes.length === checkedLength) {
					node.getParentNode().checked = true;
				}

				view.setChangeParentNodeCheckBox(setting, node.getParentNode(),
						value, srcNode);
		...
		setSonNodeCheckBox : function(setting, node, value, srcNode) {
			...
			if (node != data.getRoot(setting) && node.chkDisabled !== true) {
				...
				if (node.nocheck !== true) {
					...
					if (!hasDisable)
						node.check_Child_State = (node[childKey] && node[childKey].length > 0) ? (value ? 2
								: 0)
								: -1;
				} else {
