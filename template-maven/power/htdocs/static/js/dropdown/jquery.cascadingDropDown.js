        ...
        return this.each(function () {
            ...
            (function () {
                var methods = {
                    ...
                    reset: function () {
                        methods.clearItems();
                        $this.append('<option value="">'+config.promptText+'</option>');
                        //$(optionTag).attr("value", "").text(config.promptText)
                        $this.trigger('change');
                    },
                    initialize: function () {
                    ...
                    showLoading: function () {
                        methods.clearItems();
                        $this.append('<option value="">'+config.loadingText+'</option>'
                        		//$(optionTag).attr("value", "").text(config.loadingText)
                            );
                    },
                    loaded: function () {
                    ...
                    showError: function () {
                        methods.clearItems();
                        $this.append('<option value="">'+config.errorText+'</option>'
                        		//$(optionTag).attr("value", "").text(config.errorText)
                        		);
                    },
                    post: function () {
                        methods.showLoading();
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                methods.reset();
                                $.each(data, function () {
                                    $this.append('<option value="'+this.Value+'">'+this.Text+'</option>'
                                    		//$(optionTag).attr("value", this.Value).text(this.Text)
                                    		);
                                    $this.append('<input type="hidden" name="supAppName" value="'+this.Text+'"></input>');
                                });
                                methods.loaded();
