var selectUrl = currenthost+"/selector/groupSelectorView.htm";
function selectPerson(f_uid, f_name, isMulti){
	...
	var returnVal = window.showModalDialog(selectUrl, arg, "dialogWidth:760px;dialogHeight:540px;scroll:no;status:no;resizable:no;");
	if (typeof(returnVal) == "object") {
		if (returnVal.length > 0) {
			...
			sz_phone = sz_phone.substring(1);
			$("#"+f_uid).val(sz_id);
			$("#"+f_name).val(sz_name);
			$("#memberPhone").val(sz_phone);
		} else {
			$("#"+f_uid).val("");
			$("#"+f_name).val("");
			$("#memberPhone").val("");
