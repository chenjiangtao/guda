 ...
 */
public class UmsGatewayFactory {

    private static final Logger                logger      = LoggerFactory
                                                               .getLogger(UmsGatewayFactory.class);
    ...
    public void initGateway() {
        ...
        List<UmsGateWayInfo> gateways = gatewayService.loadValidAll();
        Iterator<UmsGateWayInfo> it = gateways.iterator();
        while (it.hasNext()) {
            UmsGateWayInfo gate = it.next();
            boolean res = false;
            try {
                res = initGate(gate);
                if (res) {
    ...
    private boolean initGate(UmsGateWayInfo gate) {
        ...
        if (GateEnum.CMPP.getValue().equalsIgnoreCase(gate.getType())) {
            ...
            //params.put("corpId", sgipMsgFactory.getCorpId());
    ...
    public boolean openGateway(String id) {
        ...
        UmsGateWayInfo gateway = gatewayService.findGateway(id);
        if (gateway == null) {
            return false;
        }
        try {
    ...
    private void runMonitorTask() {
        ...
        while (tt.hasNext()) {
            ...
            if (GateEnum.CMPP == type) {
                ...
                CMPPSMProxy proxy = (CMPPSMProxy) gateway.get(key);
                ...
                String key = gEntry.getKey();
                CMPP30SMProxy proxy = (CMPP30SMProxy) gateway.get(key);
                if (proxy == null || proxy.isClosed()) {
                    if (proxy != null) {
                        proxy.close();
                    }
    ...
    private void restart(String key) {
        UmsGateWayInfo gate = gatewayService.findGateway(key);
