...
public class AccessFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        URL url = currentClassLoader.getResource("security/ums.acf");
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        String temppath = httpRequest.getServletPath();
        String realPath = tempurl.substring(0, tempurl.indexOf(temppath));
        logger.info("request:" + path + ",contentpath:" + contextPath + ",realpath:" + realPath
                    + ",上下文:" + bo);
        if (context == null) {
            if (getRole(path) != null) {
                if (!"login.htm".equals(path)) {
                    httpResponse.sendRedirect(VelocityToolboxView.getReqHost(httpRequest)
                                              + "/login.htm?errorCode=needLogin&refer="
                                              + getRefer(path, queryStr));
                    return;
                }
            } else {
