 ...
 * 
 * @author yangjuanying
 * @version $Id: SelectMsgOutBizImpl.java, v 0.1 2012-12-11 下午8:49:08 yangjuanying Exp $
 */
public class SelectMsgOutBizImpl implements SelectMsgOutBiz {

    private static final Logger  logger = Logger.getLogger(SelectMsgOutBizImpl.class);
     ...
     * 条件分页【短信查询用户短信】
     * 
     * @see net.zoneland.ums.biz.config.queryadmin.SelectMsgOutBiz#selectStatMsgOut(net.zoneland.ums.common.dal.bo.AppMsgInfoBO, int)
     */
    public PageResult<UmsMsgOut> selectStatMsgOut(AppMsgInfoBO bo, int curPage) {
        logger.debug("开始【短信查询用户短信】");
        ...
        total = umsMsgOutMapper.queryMsgOutNum(params);// 记录总数量
        result = new PageResult<UmsMsgOut>(total, curPage);
        params.put("orderBy", "GMT_MODIFIED");
        params.put("first", result.getFirstrecode());
        params.put("end", result.getEndrecode());
        List<UmsMsgOut> msgList = umsMsgOutMapper.queryMsgOutByPage(params);// 分页显示【短信查询用户短信】
